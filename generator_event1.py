...
def add_labels(g,
               story):  # This methods add to our output story graph all the labels of the instances involved in the story. They are necessary for the visualization!
...
def instantiate_ordinary_world(g, fixed):
    ...
    qres = g.query(ordinary_world_template.substitute({'hero': fixed["Hero"].split("/")[-1]}),
                   initNs={'HERO': 'http://hero_ontology/'})
...
def create_dic(story):

    text1 = list(textGeneration_Event1(story))
    text2 = textGeneration_Event12(story)
    text3 = textGeneration_Event13(story)
    text1_list = []
    text2_list = []
    text3_list = []
    ...
    dict = {}
    dict['Event_11'] = text1_list
    dict['Event_12'] = text2_list
    dict['Event_13'] = text3_list
    dict['Knowledge Graph'] = clean(story)
...
def main(argv, arc):
    ...
    data = {}
    for i in range(5):
        story_dict = main_run('random')  # story is a dictionary with triples and text
        print(story_dict['Event_11'])
        #if story['Event_11']==  []:
            #next(it)
